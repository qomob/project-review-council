...
📍 [examples/](./examples/)：[startup](./examples/startup-example.md) / [saas](./examples/saas-example.md) / [github](./examples/github-example.md) — 展示预期输出风格与证据标注规范
...
- ✅ 角色评分趋同检测已接入 P3 Synthesizer（极差<2 或 标准差<1.0 触发独立性重审）
- 待补充：证据不足时的降级策略
- 待补充：小型项目过度审计的裁剪规则
